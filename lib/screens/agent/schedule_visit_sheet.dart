...
class ScheduleVisitSheet extends StatefulWidget {
  ...
  const ScheduleVisitSheet({super.key, required this.customer});
...
class _ScheduleVisitSheetState extends State<ScheduleVisitSheet> {
  ...
  void initState() {
    super.initState();
    final DateTime? scheduled = widget.customer.scheduledVisit;
    if (scheduled != null) {
      _selectedDate = scheduled;
      _focusedMonth = scheduled;

      final int hour = scheduled.hour;
      if (hour >= 12 && hour < 15) {
        _selectedSlot = 'AFTERNOON';
      } else if (hour >= 15) {
        _selectedSlot = 'EVENING';
      } else {
        _selectedSlot = 'MORNING';
      }
    }

    final String remarks = widget.customer.assignment?.remarks ?? '';
    if (remarks.isNotEmpty) {
      if (remarks.contains('Purpose: ')) {
        _purposeController.text = remarks.split('Purpose: ').last;
      } else if (!remarks.contains('Scheduled follow-up visit')) {
        _purposeController.text = remarks;
      }
    }
  }
  ...
  void _handleConfirm() async {
    final String timeSlotText = _slotsInfo.firstWhere(
      (s) => s['id'] == _selectedSlot,
    )['window'];
    final String visitNote =
        'Scheduled follow-up visit for ${_selectedDate.day}/${_selectedDate.month}/${_selectedDate.year} during $timeSlotText.';
    ...
    final DateTime scheduledDateWithTime = DateTime(
      _selectedDate.year,
      _selectedDate.month,
      _selectedDate.day,
      hour,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      padding: EdgeInsets.fromLTRB(20, 16, 20, 24 + keyboardOffset),
      child: SafeArea(
        top: false,
        child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            '${_monthsNames[_focusedMonth.month - 1]} ${_focusedMonth.year}'
                                .toUpperCase(),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText:
                            'e.g. Confirm online bank transfer transaction, or collect balance dues...',
                        hintStyle: TextStyle(
